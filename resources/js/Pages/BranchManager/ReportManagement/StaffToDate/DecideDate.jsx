...
export default function ReportOverview({prefix,what,type,userName,allData}){
    ...
    const {data,setData,post,processing, errors,clearErrors, reset, selectedDate,setSelectedDate, pageMinWidth,pageMaxWidth}=useDecideDateDefinitions();
    
    const {}=useDecideDateActions();
    ...
    {Object.entries(allData).map(([staffId,eachDataByStaff],index)=>
        <div className={`base_frame base_backColor border-black border rounded-sm ${pageMaxWidth} ${pageMinWidth} my-2`}  key={staffId}>
            ...
            <OverviewTable {...{allData:eachDataByStaff.each_data_by_staff,pageMaxWidth,pageMinWidth}}/>
        </div>
    )}
