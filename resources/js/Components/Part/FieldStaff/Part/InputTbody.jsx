...
// 報告書入力のtableの中身
export default function InputTbody({mainProjectName,projectSets,eachData,trIndex,widthSets,onAssignedInputChange,inputRefs,inputValues,isConfirm}){
        ...
        return(
                <tr className="border-black border-2 base_backColor" key={trIndex}>
                    ...
                    <td className={`border-black border-2  ${widthSets[1]}`}>{eachData.household}</td>
                    ...
                    {Object.keys(projectSets).map(function(eachSet,index){
                        ...
                        return(
                            ...
                            <td key={"sub" + index} className={`border-black border-2 ${widthSets[index+2]}`}>
