...
class CSVExportFlow{
    ...
    public static function create_reports_csv_flow($project_ids){
        ...
        [$plans_in_project_ids,$address_sets,$distribution_record_sets]=GetDataInSql::get_detailed_planned_data_by_project_ids($project_ids);

        // プランにあるデータ(案件=>[[町目名:,部数:日付:(複数日の場合あり),スタッフ:(複数人の場合あり)]の入れ子配列])
        //CSVのためにもう1ついくか？
        $formatted_projects_and_records_data=FormatData::data_change_for_csv_report_data($plans_in_project_ids,$address_sets,$distribution_record_sets);
